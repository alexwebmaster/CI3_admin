...
class Locations extends Admin_Controller {	
	...
	function index()
	{
		...
		$this->view($this->config->item('admin_folder').'/settings/countries/list', $data);
	}
	...
	function country_form($id = false)
	{
		...
		{
			$this->view($this->config->item('admin_folder').'/settings/countries/country_form', $data);
		}
		else
		{
			$save['id']					= $id;
	...
	function zones($country_id)
	{
		...
		$this->view($this->config->item('admin_folder').'/settings/countries/country_zones', $data);
	}
	...
	function zone_form($id = false)
	{
		...
		if ($this->form_validation->run() == FALSE)
		{
			$this->view($this->config->item('admin_folder').'/settings/countries/country_zone_form', $data);
		}
		else
		{
			$save['id']			= $id;
	...
	function get_zone_areas()
	{
		...
		$areas	= $this->Location_model->get_zone_areas_menu($id);
		var_dump($areas);
	...
	function zone_areas($id)
	{
		...
		$this->view($this->config->item('admin_folder').'/settings/countries/country_zone_areas', $data);
	}
	...
	function zone_area_form($zone_id, $area_id =false)
	{
		...
		if ($this->form_validation->run() == FALSE)
		{
			$this->view($this->config->item('admin_folder').'/settings/countries/country_zone_area_form', $data);
		}
		else
		{
			$save['id']			= $area_id;
