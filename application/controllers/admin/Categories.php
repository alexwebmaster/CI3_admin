...
class Categories extends Admin_Controller {	
	...
	function index()
	{
		...
		$this->view($this->config->item('admin_folder').'/categories/list', $data);
	}
	...
	function form($id = false)
	{
		...
		$data['seo_title']		= '';
		$data['meta']			= '';
		$data['parent_id']		= $this->input->get('parent_id', 0);
		$data['error']			= '';
		$data['enabled']		= 1;
		$data['qualified']		= 1;
		$data['show_categories']			= 'list';
		$data['show_description']			= 0;
		$data['fields']			= $this->config->item('fields');
		$data['field_types']	= $this->config->item('field_types');
		...
		{	
			...
			$data['qualified']		= $category->qualified;
			$data['show_categories']			= $category->show_categories;
			$data['show_description']			= $category->show_description;
			$category_fields		= json_decode($category->fields, true);
			if (!empty($category_fields)) {
				$data['fields']			= $category_fields;
			}
		}
		...
		{
			$this->view($this->config->item('admin_folder').'/categories/form', $data);
		}
		else
		{
			...
			if($uploaded)
			{
				...
				//delete old image
				@unlink('uploads/images/full/'.$category->image);
				@unlink('uploads/images/medium/'.$category->image);
				@unlink('uploads/images/small/'.$category->image);
				@unlink('uploads/images/thumbnails/'.$category->image);
			...
			$save['qualified']		= $this->input->post('qualified');
			$save['show_categories']			= $this->input->post('show_categories');
			$save['show_description']			= $this->input->post('show_description');

			$save['route_id']		= intval($route_id);
			$save['slug']			= $slug;

			$fields					= array();
			$base_field				= $this->config->item('base_field');

			foreach ($this->input->post('field') as $key => $f)
			{
				$fields[$key]	= array_merge($base_field, array( 'order' => $_POST['order'][$key], 
										'field' => $_POST['field'][$key],
										'title' => $_POST['title'][$key], 
										'display' => $_POST['display'][$key], 
