...
<ul class="nav nav-tabs" id="Deliveries_Tab">
<?php foreach ($statuses as $s):?>
	<li <?php echo ($s == 'Delivered SG')? 'class="active"' : ''; ?>>
		<a href="#deliveries_container" onclick="get_pane2('<?php echo md5($s); ?>')"><?php echo $status[$s]; ?></a>
	</li>
<?php endforeach; ?>
</ul>
...
<script>
	function get_pane2(id)
	{
		$('#deliveries_container .tab-pane').hide();
		$('#'+id).show();
	}
	
	$(function () {
		$('#Deliveries_Tab a').click(function (e) {
		  e.preventDefault();
		  $(this).tab('show');
		});
		$('#Deliveries_Tab a').first().click();
	});
</script>