<?php echo form_open($this->config->item('admin_folder').'/pages/link_form/'.$id); ?>
	...
	<?php
	$data	= array('id'=>'title', 'name'=>'title', 'value'=>set_value('title', $title), 'class'=>'col-3');
	echo form_input($data);
	?>

	<label for="url"><?php echo lang('url');?></label>
	<?php
	$data	= array('id'=>'url', 'name'=>'url', 'value'=>set_value('url', $url), 'class'=>'col-3'); 
	echo form_input($data);
	?>
	...
	echo form_dropdown('parent_id', $options,  set_value('parent_id', $parent_id, 'class="col-3 custom-select"'));
	?>

	<label for="sequence"><?php echo lang('sequence');?></label>
	<?php
	$data	= array('id'=>'sequence', 'name'=>'sequence', 'value'=>set_value('sequence', $sequence), 'class'=>'col-3');
	echo form_input($data);
	?>
