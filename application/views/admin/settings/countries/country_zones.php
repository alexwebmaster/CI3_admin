...
<?php foreach ($zones as $location):?>
		<tr>
			...
			<td>
				<div class="btn-group" style="float:right;">
					<a class="btn btn-primary" href="<?php echo site_url($this->config->item('admin_folder').'/locations/zone_form/'.$location->id); ?>"><i class="fas fa-pencil-alt"></i> <?php echo lang('edit');?></a>
					<a class="btn btn-primary" href="<?php echo site_url($this->config->item('admin_folder').'/locations/zone_areas/'.$location->id); ?>"><i class="fas fa-map-marker"></i> <?php echo lang('zone_areas');?></a>
					<a class="btn btn-danger" href="<?php echo site_url($this->config->item('admin_folder').'/locations/delete_zone/'.$location->id); ?>" onclick="return areyousure();"><i class="fas fa-trash"></i> <?php echo lang('delete');?></a>
				</div>
			</td>
	  </tr>
<?php endforeach; ?>
